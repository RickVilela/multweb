  ...
  <LayoutAuthenticated>
    <SectionMain>
      <SectionTitleLineWithButton :icon="mdiBallotOutline" title="Cadastro de Fornecedor" main>
        <BaseButton v-if="!showForm.novo" @click="toggleForm" label="Novo Fornecedor" color="success" :icon="mdiAccountPlus"/>
        <BaseButton v-else @click="toggleForm" label="Ver Fornecedores" color="info" :icon="mdiAccountEye"/>
          ...
          <template #empty>
            <FormTabs class="overflow-x-auto text-base sm:text-sm md:text-base lg:text-lg whitespace-nowrap">
              ...
              <FormTab name="dados_adicionais" label="Dados Adicionais"
                :elements="['ramo_atividade', 'transportadora', 'frete_por_conta', 'plano_contas', 'previsao_entrega', 'button_submit']"
                :columns="{ sm: 12, md: 6, lg: 4 }" />
              ...
              <FormTab name="configuracoes" label="Configurações"
                :elements="['situacao', 'tipo_contribuinte', 'finalidade_faturamento', 'button_submit']"
                :columns="{ sm: 12, md: 6, lg: 4 }" />
              </FormTabs>
            ...
            <FormElements>
              <!-- Início Informações Principais-->
                <SelectElement name="pessoa" label="Pessoa" :native="false" :items="[
                  'Física',
                  'Jurídica',
                  'Estrageiro(a)',
                ]" :columns="6" />
              ...
              <!-- Início Endereços-->
                ...
                <SelectElement name="tipo_endereco" label="Tipo" :native="false" :items="[
                  'Faturamento',
                  'Entrega',
                  'Residência',
                ]" :columns="6" />
              ...
              <!-- Início Telefones-->
                <ListElement name="telefones" add-text="+" :sort="true" :override-classes="{
                  ...
                  <template #default="{ index }">
                      ...
                      <SelectElement name="tipo_telefone" label="Tipo" :native="false" :items="[
                        'Whatsapp',
                        'Celular',
                        'Telefone',
                      ]" :columns="3" />
              ...
              <!--Início Dados Adicionais-->
                  <SelectElement name="ramo_atividade" :search="true" label="Ramo de Atividade" :native="false" :items="[
                    '',

                  ]" :columns="4" />

                  <SelectElement name="transportadora" :search="true" label="Transportadora" :native="false" :items="[
                    '',
                  ]" :columns="4" />

                  <SelectElement name="frete_por_conta" :search="true" label="Frete por conta" :native="false" :items="[
                    '',
                  ]" :columns="4" />

                  <SelectElement name="plano_contas" :search="true" label="Plano de contas" :native="false" :items="[
                    '',
                  ]" :columns="6" />

                  <TextElement name="previsao_entrega" label="Previsão de entrega (Dias)" input-type="number" :columns="6" />
                ...
                <ListElement name="dados_financeiros" add-text="+" :sort="true" :override-classes="{
                  ...
                  <template #default="{ index }">
                    <ObjectElement :name="index" add-class="mb-4" :override-classes="{
                      ...
                      <SelectElement name="banco" :search="true" label="Banco" :native="false" :items="[
                        '',
                      ]" :columns="6" />
                      ...
                      <SelectElement name="tipo" :search="true" label="Tipo" :native="false" :items="[
                        '',
                      ]" :columns="12" />
              ...
              <!--Início Configurações-->
                <SelectElement name="situacao" default="Ativo" label="Situação" :native="false" :items="[
                  'Ativo',
                  'Inativo'
                ]" :columns="4" />

                <SelectElement name="tipo_contribuinte" :search="true" label="Tipo contribuinte" :native="false" :items="[
